...
http://blog.kkcpct.org/#contact`
		...
		LineTemplate_family_focus := linebot.NewCarouselTemplate(
			...
			linebot.NewCarouselColumn(
				...
				linebot.NewPostbackTemplateAction("搭車者聯絡登記","聯絡資訊 POST", "聯絡資訊"),
		...
		obj_message_family_focus := linebot.NewTemplateMessage(t_family_focus, LineTemplate_family_focus)
					...
					ImageURL_week_1 := "https://trello-attachments.s3.amazonaws.com/5897c415be467e461197bfbd/5a7f83f1ac746c82d2f59b0a/a65d568042f31a1d6362cb507723dc28/20180211.index.jpg"
					...
					this_week_text_1 := `本週週報（2018/02/11）：
https://s.kkcpct.org/2H74T4W
...
// `
		...
		next_week_msg := `本週主日預告（2017/08/27）
預定行事：
※ 升級開學祝福聯合禮拜
※ 聖禮典（聖餐、洗禮）
※ 定期小會
...
iCalendar Feeds 使用方法請參照：https://s.kkcpct.org/2mbZYER`
		...
		LineTemplate_nextweek_review := linebot.NewCarouselTemplate(
			...
			linebot.NewCarouselColumn(
				Bible_imageURL, "主日禮拜經文預習", "以下是本週（2018/02/11）的經文",
//  				linebot.NewPostbackTemplateAction("聯合禮拜 提摩太前書 4:6-15","第一場預習","聖經 提摩太前書 4:6-15"),
 				linebot.NewPostbackTemplateAction("台語禮拜 歌羅西書 3:23-24","第一場預習","聖經 歌羅西書 3:23-24"),
				 linebot.NewPostbackTemplateAction("華語禮拜 使徒行傳 8:26-31","第一場預習","聖經 使徒行傳 8:26-31"),
...
//  				linebot.NewPostbackTemplateAction("本週新眼光","新眼光 POST", "新眼光"),
		...
		LineTemplate_nextweek_read := linebot.NewCarouselTemplate(
			linebot.NewCarouselColumn(
				Bible_imageURL, "本週新眼光讀經", "2017/08/27～2017/08/29",
				linebot.NewPostbackTemplateAction("27 日 歷代志下 33:21-34:7","新眼光查經","聖經 歷代志下 33:21-34:7"),
				linebot.NewPostbackTemplateAction("28 日 歷代志下 34:8-28","新眼光查經","聖經 歷代志下 34:8-28"),
				linebot.NewPostbackTemplateAction("29 日 歷代志下 34:29-33","新眼光查經","聖經 歷代志下 34:29-33"),
			),
			linebot.NewCarouselColumn(
				Bible_imageURL, "本週新眼光讀經", "2017/08/30～2017/09/01",
				linebot.NewPostbackTemplateAction("30日 歷代志下 35:1-19","新眼光查經","聖經 歷代志下 35:1-19"),
				linebot.NewPostbackTemplateAction("31日 歷代志下 35:20-26","新眼光查經","聖經 歷代志下 35:20-26"),
				linebot.NewPostbackTemplateAction("1日 歷代志下 36:1-8","新眼光查經","聖經 歷代志下 36:1-8"),

			),
			linebot.NewCarouselColumn(
				Bible_imageURL, "本週新眼光讀經", "2017/09/02",
				linebot.NewPostbackTemplateAction("2日 歷代志下 36:9-10","新眼光查經","聖經 歷代志下 36:9-10"),
...
2 日 歷代志下 36:9-10` + "\n\n提示：\n這部分在最新版本 LINE APP 會以預習經文按鈕呈現，\n可幫助會眾快速熟悉經文。"
		...
		if event.Type == linebot.EventTypePostback {
				...
				if event.Postback.Data == "週報"{
					...
					//if _, err = bot.ReplyMessage(event.ReplyToken, obj_message_nextweek_read,linebot.NewTextMessage(next_week_msg),obj_message_week_1,obj_message_nextweek_review).Do(); err != nil {
					if _, err = bot.ReplyMessage(event.ReplyToken, obj_message_nextweek_review,obj_message_week_1,linebot.NewTextMessage(this_week_text_1)).Do(); err != nil {//obj_message_event,linebot.NewTextMessage(event_text)).Do(); err != nil {
							log.Print(486)
							log.Print(err)
					}
				}
...
//							if _, err = bot.ReplyMessage(event.ReplyToken, linebot.NewTextMessage("本週教會電子週報從缺\n這只是示範效果可以如何展示"),obj_message_week_2,obj_message_week_3,linebot.NewTextMessage(next_week_msg),obj_message_nextweek_review).Do(); err != nil {
							//if _, err = bot.ReplyMessage(event.ReplyToken, linebot.NewTextMessage(this_week_text_1),obj_message_nextweek_read,linebot.NewTextMessage(next_week_msg),obj_message_nextweek_review).Do(); err != nil {
							...
							if _, err = bot.ReplyMessage(event.ReplyToken, obj_message_nextweek_review,obj_message_week_1,linebot.NewTextMessage(this_week_text_1)).Do(); err != nil {
									log.Print(7161)
									log.Print(err)
							}
							return
